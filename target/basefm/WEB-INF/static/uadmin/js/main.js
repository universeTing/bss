$(function () {
    //BEGIN MENU SIDEBAR
    $('.sidebar-default #sidebar').addClass("custom-scrollable").css({'height': window.innerHeight-50+"px"});
    ...
    $(window).on("load resize", function () {
        if ($(this).width() < 768) {
            ...
            $('body.sidebar-default div.sidebar-collapse,body.sidebar-colors div.sidebar-collapse').addClass("custom-scrollable").css({'height': window.innerHeight-50+"px","overflow-y":"auto"});
        }

        if($('#sidebar').height() > $('#page-wrapper').height()){
            $('#wrapper').css('height', $('#sidebar').height());
        }
        $('.sidebar-default #sidebar').addClass("custom-scrollable").css({'height': window.innerHeight-50});
    });
