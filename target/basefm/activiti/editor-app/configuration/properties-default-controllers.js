...
var KisBpmStringPropertyCtrl = [ '$scope', function ($scope) {
    ...
    //如果为流程唯一标识，则不允许修改  @bgg
    $scope.inputFocusred = function(th){
    	//alert($scope.property.key)//oryx-overrideid
    	if($scope.property.key === 'oryx-process_id' //流程唯一标识
    		|| $scope.property.key === 'oryx-overrideid'//主键ID
    		|| $scope.property.key === 'oryx-name'//名称
    			){
...
var KisBpmTextPropertyCtrl = [ '$scope', '$modal', function($scope, $modal) {
    ...
    var opts = {
        template:  'activiti/editor-app/configuration/properties/text-popup.html?version=' + Date.now(),
        scope: $scope
    };
