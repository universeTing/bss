 ...
 */
@Controller
@RequestMapping("${admin.url.prefix}/workflow/actretype")
public class ActReTypeController extends BaseBeanController<ActReType> {

    @Autowired
    protected IActReTypeService actReTypeService;

    public ActReType get(String id) {
        if (!ObjectUtils.isNullOrEmpty(id)) {
            return actReTypeService.selectById(id);
        } else {
            return newModel();
        }
    }
	...
	public void treeData(Queryable queryable,
		...
		EntityWrapper<ActReType> entityWrapper = new EntityWrapper<ActReType>(entityClass);
		entityWrapper.setTableAlias("t.");
		List<ActReType> treeNodeList = null;
		if (!async) { // 非异步 查自己和子子孙孙
			treeNodeList = actReTypeService.selectTreeList(queryable, entityWrapper);
		...
		PageJson<ActReType> pagejson = new PageJson<ActReType>(treeNodeList);
	...
	private void bootstrapTreeData(Queryable queryable,
		...
		EntityWrapper<ActReType> entityWrapper = new EntityWrapper<ActReType>(entityClass);
		entityWrapper.setTableAlias("t.");
		List<ActReType> treeNodeList = actReTypeService.selectTreeList(queryable, entityWrapper);
    ...
    @ResponseBody
    public AjaxJson create(Model model, @Valid @ModelAttribute("data") ActReType actReType, BindingResult result,
                           HttpServletRequest request, HttpServletResponse response) {
        return doSave(actReType, request, response, result);
    }

    @RequestMapping(value = "{id}/update", method = RequestMethod.GET)
    public String update(@PathVariable("id") String id, Model model, HttpServletRequest request,
                              HttpServletResponse response) {
        ActReType actReType = get(id);
        model.addAttribute("data", actReType);
        return display("edit");
    }

    @RequestMapping(value = "{id}/update", method = RequestMethod.POST)
    @ResponseBody
    public AjaxJson update(Model model, @Valid @ModelAttribute("data") ActReType actReType, BindingResult result,
                           HttpServletRequest request, HttpServletResponse response) {
        return doSave(actReType, request, response, result);
    }
    ...
    @ResponseBody
    public AjaxJson doSave(ActReType actReType, HttpServletRequest request, HttpServletResponse response,
                           BindingResult result) {
        AjaxJson ajaxJson = new AjaxJson();
        ajaxJson.success("保存成功");
        if (hasError(actReType, result)) {
            // 错误提示
        ...
        try {
            if (StringUtils.isEmpty(actReType.getId())) {
                actReTypeService.insert(actReType);
            } else {
                actReTypeService.insertOrUpdate(actReType);
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public AjaxJson delete(@PathVariable("id") String id) {
        ...
        try {
            actReTypeService.deleteById(id);
        } catch (Exception e) {
            e.printStackTrace();
            ajaxJson.fail("删除失败");
        }
    ...
    public AjaxJson batchDelete(@RequestParam(value = "ids", required = false) String[] ids) {
        ...
        try {
            ...
            actReTypeService.deleteBatchIds(idList);
        } catch (Exception e) {
            e.printStackTrace();
            ajaxJson.fail("删除失败");
        }
    ...
    public ValidJson validate(DuplicateValid duplicateValid, HttpServletRequest request) {
        ...
        try {
            EntityWrapper<ActReType> entityWrapper = new EntityWrapper<ActReType>(entityClass);
            valid = actReTypeService.doValid(duplicateValid,entityWrapper);
            if (valid) {
