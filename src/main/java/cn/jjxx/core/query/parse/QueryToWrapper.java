package cn.jjxx.core.query.parse;
...
import cn.jjxx.core.query.data.Condition;
import cn.jjxx.core.query.data.Queryable;
import cn.jjxx.core.query.data.Condition.Filter;
import cn.jjxx.core.query.data.Condition.Operator;
...
public class QueryToWrapper<T> {
	...
	public void parseSort(Wrapper<T> wrapper, Queryable queryable) {
		cn.jjxx.core.query.data.Sort sort = queryable.getPageable().getSort();
		if (sort != null) {
			for (cn.jjxx.core.query.data.Sort.Order order : sort) {
				if (order.getDirection() == cn.jjxx.core.query.data.Sort.Direction.DESC) {
					wrapper.orderBy(order.getProperty(), false);
				} else if (order.getDirection() == cn.jjxx.core.query.data.Sort.Direction.ASC) {
