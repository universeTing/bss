package cn.jjxx.core.query.wrapper;
...
public class EntityWrapper<T> extends Wrapper<T> {
	...
	/**
	 * queryFieldMap query用来完善查询对象的对象的属性进行查询的方法 by zhoukai 2017-11-22
	 */
	private Map<String, String> queryFieldMap;
	...
	private String handleColumn(String column) {
		...
		// 处理对象的对象的属性 by zhoukai 2017-11-22
		if (column != null && column.contains(".") && !column.trim().equals("1")) {
			String[] columns = column.split("\\.");
			String newColumn ="";
			if(columns.length > 1 && queryFieldMap!=null){
				newColumn = queryFieldMap.get(columns[0]);
				if(!cn.jjxx.core.utils.StringUtils.isEmpty(newColumn)){
					column = newColumn+'.'+ columns[1];
				}	
			}				
		}
		return column;
	...
	public Map<String, String> getQueryFieldMap() {
		return queryFieldMap;
	}

	public void setQueryFieldMap(Map<String, String> queryFieldMap) {
