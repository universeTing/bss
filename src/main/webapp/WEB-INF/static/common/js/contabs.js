 ...
 * @author bbg.<br>
 */
function menuChange(index,icon,name){
	$("#nav-list-ul").html(menus[index-1]);
	$("#sidebar-shortcuts").html('<i class="ace-icon fa '+icon+'"></i>'+name);
	/*动态添加html标签后，需重新绑定点击事件，方可执行  @bgg*/
...
function menuItem() {
    ...
    var nid = $(this).data("menuid");
    if(nid){
		if (window.localStorage) {
			localStorage.setItem("nid", nid);	
		} else {
			Cookie.write("nid",nid);	
		}
	}
    ...
    // 选项卡菜单不存在
    if (flag) {
        var str = '<a href="javascript:;" class="active J_menuTab" data-id="' + dataUrl + '">' + menuName + ' <i class="iconfont icon-error"></i></a>';
